...
- `contract_index`: `status_semantic`(契約状態。有効=`active`)、`staff_page_ids` / `search_text` / `billing_terms` を保持(マイグレーション `20260807100000_phase6_contract_complaint_index.sql`)。
- `complaint_index`: 本文はキャッシュせず `summary` / `body_hash`。`status_semantic`(未解決=`open`|`in_progress`、完了=`done`)、`staff_page_id` / `search_text` を保持(同マイグレーション)。
