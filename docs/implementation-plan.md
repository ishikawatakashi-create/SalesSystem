...
- 2026-08-09 Phase 13A(Prospect Pool / 営業リスト基盤): Supabase に営業候補・リスト・CSV取込・割当。Notion 非書込。詳細は [phase13a-prospect-pool.md](./phase13a-prospect-pool.md)。
- 2026-08-09 Phase 13B(架電管理 / Prospect昇格): call_attempts・架電キュー・claim・正式Organization昇格。詳細は [phase13b-call-promotion.md](./phase13b-call-promotion.md)。
...
## Phase 11: お問い合わせ受信箱 / Apps Script 取込(実装完了・人間設定後に実E2E)

**ゴール: Strikingly 通知を Gmail label + Apps Script 5分 poll で取り込み、受信箱で振分け・顧客/履歴化できる。**

1. Supabase `inquiries` + HMAC ingest endpoint — **完了**
2. `/inquiries`・triage・顧客/担当/履歴化 — **完了**
3. Apps Script artifact（`integrations/apps-script/strikingly-inquiries/`）— **完了**
4. Pub/Sub / Gmail OAuth transport は廃止（DB は破壊せず残置）— **完了**
5. Apps Script / Gmail label 人間設定・実E2E — **checkpoint（運用側）**
6. 返信送信・AI分類 — **非対象**
