...
改訂履歴: 2026-08-05 設計レビュー反映(ジョブ基盤・スケジューラー・分散レート制御をPhase 1へ移動、認証スパイク追加、write_operations等の新規要素反映)。
...
1. 正規化ライブラリ `lib/normalize/`+単体テスト
2. 書込パイプライン `lib/sync/write-pipeline.ts`(**write_operations+external_idによる冪等化・クラッシュ復旧**、部分失敗のsync_errors記録、楽観ロック)+統合テスト
3. 顧客登録・編集(RHF+Zod、masters_cache参照)
4. 顧客検索・一覧(検索・フィルタ・AND/OR・TanStack Table・列設定・固定列・URL条件保存・プレビューパネル)
5. 顧客詳細(ヘッダー・基礎情報枠・営業枠、Notion正本取得+短時間キャッシュ、同期警告バナー)
6. 関連アカウント(customer_relations+双方向表示)
7. 顧客担当者(登録・編集・無効化)+**再計算ハンドラー(顧客・顧客担当者)**: 担当者名変更時のcustomer_index.search_text再構築。書込パイプライン内の同期実行とし、Phase 4のジョブ化・管理画面に依存しない([sync-design.md §7 実装順](./sync-design.md#実装順phaseとの対応))
8. 保存済み検索・最近閲覧
9. マイデスク(担当顧客・最近閲覧/更新・新規登録導線。KPI・アクション枠はPhase 3)
10. E2E: 顧客検索 / 詳細 / 登録 / 編集 / 表記揺れ検索

## Phase 3: 営業活動

**ゴール: 案件・対応履歴(複数分類・一括登録)・次回アクション・契約・クレームが動く。**

1. 案件(登録・編集・一覧。status_semanticの解決)
2. 対応履歴
   - 登録・編集(複数分類、登録者/最終編集者、**本文はページ本文ブロック+要約プロパティ**、本文ハッシュ監査、本文競合の楽観ロック)
   - タイムライン表示(コンパクト1行・クリック展開・本文遅延取得・絞り込み)
   - 削除機能を実装しないことの担保(UI・API・テスト)
3. **次回アクション(次回アクションDB+action_index)**
   - 履歴登録フォームからのアクション自動生成、単独作成、完了・取消
   - **再計算ハンドラー(対応履歴・次回アクション・案件)**: 顧客の最新対応内容・最終対応日、顧客・案件のnext_action / next_action_date、顧客.見込み金額(進行中・保留案件の合計)。書込パイプライン内の同期実行
   - マイデスクの本日予定・期限超過をaction_index参照へ
4. 一括履歴登録(複数選択→確認画面→batch_id+決定的external_idで顧客ごとに独立作成。多件数はジョブ化)
5. 契約(専用セクション。契約書URL/ファイル別プロパティ)
6. クレーム(専用セクション。詳細本文は見出し付きページ本文)
7. **顧客詳細「全活動」タイムライン**(対応履歴・案件受注・契約作成・クレーム発生/解決を表示上のみ統合。[ui-guidelines.md](./ui-guidelines.md))
8. マイデスクKPI完成(semantic_keyベースの集計、クリックで条件付き一覧へ)
9. ユーザープロフィール画面
10. E2E: 履歴登録 / 複数分類 / 一括登録 / アクション完了 / 案件 / 契約 / クレーム / 履歴削除不可
...
2. Notion Webhook(署名検証+**1トランザクションenqueue**(ingest_webhook_event)+後続同期ジョブ+自書込スキップ)
