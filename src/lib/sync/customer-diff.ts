...
export function buildCustomerPropertyDiff(input: {
  ...
  const beforeProps = customerToNotionProperties({
    customer: {
      ...
      relationshipPageIds: input.before.relationshipPageIds ?? [],
...
export function buildChangedFieldsAudit(input: {
  ...
  const pairs: Array<[string, unknown, unknown]> = [
    ...
    [
      "関係性",
      input.before.relationshipPageIds ?? [],
      input.write.relationshipPageIds ?? [],
    ],
...
export function writeInputToDomainFields(
  ...
  return {
    ...
    relationshipPageIds: write.relationshipPageIds ?? [],
