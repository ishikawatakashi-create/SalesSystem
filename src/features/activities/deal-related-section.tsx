...
export function DealRelatedSection({
  dealPageId,
  customerPageId,
  activities,
  activityLabels,
  ...
  const nextOverdue = overdueDaysTokyo(derivedNext.nextActionDate);
  ...
  return (
    <div className="space-y-3">
      <section className="rounded border border-slate-200 bg-white p-3">
        ...
        <dl className="divide-y divide-slate-100">
          <div className="flex gap-2 py-1">
            <dt className="w-28 shrink-0 text-xs text-slate-500">内容</dt>
          ...
          <div className="flex gap-2 py-1">
            <dt className="w-28 shrink-0 text-xs text-slate-500">予定日</dt>
            <dd
              className={`text-xs ${nextOverdue ? "font-medium text-red-700" : ""}`}
            >
              {formatActionDate(derivedNext.nextActionDate)}
            </dd>
          </div>
        </dl>
      </section>
      ...
      <section className="rounded border border-slate-200 bg-white">
        <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 px-3 py-2">
          <h2 className="text-xs font-bold text-slate-700">次回アクション</h2>
          <span className="text-xs text-slate-500">{actions.length}件</span>
          {canEditAction && customerPageId && (
            <Link
              href={`/deals/${dealPageId}/actions/new`}
              ...
              {actions.slice(0, 20).map((row) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-2 py-1.5">
                      {row.status_id
                        ? (actionLabels.statusNames.get(row.status_id) ??
                          (row.is_open ? "未完了" : "—"))
                        : row.is_open
                          ? "未完了"
                          : "—"}
