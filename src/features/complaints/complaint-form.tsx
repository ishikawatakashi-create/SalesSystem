...
import { ComplaintFormFields } from "@/features/complaints/complaint-form-fields";
import {
  complaintClientFormSchema,
  toComplaintFormDefaults,
  type ComplaintClientFormParsed,
  type ComplaintClientFormValues,
} from "@/features/complaints/form-schema";
import type { ComplaintFormOptions } from "@/features/complaints/options";

export type ComplaintFormInitial = Parameters<
  typeof toComplaintFormDefaults
>[0];

export type ComplaintFormMeta =
...
export function ComplaintForm({
  ...
  meta: ComplaintFormMeta;
  options: ComplaintFormOptions;
  initial?: ComplaintFormInitial;
  ...
  const defaults = useMemo(() => {
    const base = toComplaintFormDefaults(initial, lockedId);
    if (lockedDealPageId) {
  ...
  } = useForm<ComplaintClientFormValues, unknown, ComplaintClientFormParsed>({
    resolver: zodResolver(complaintClientFormSchema),
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    let result: ComplaintActionResult;
    if (meta.mode === "create") {
      result = await createComplaintAction({ requestId, data: payload });
    } else {
      result = await updateComplaintAction({
        requestId,
    ...
    if (result.ok) {
      const dest =
        meta.successRedirect ?? `/complaints/${result.notionPageId}?saved=1`;
      router.push(dest);
      router.refresh();
      return;
    }
